...
def correlation(x:torch.Tensor, y:torch.Tensor) -> float:
    """
    Correlates two 1D tensors
    """
    x = x - x.mean()
    y = y - y.mean()
    denom = torch.sqrt((x ** 2).sum()) * torch.sqrt((y ** 2).sum())
    if denom==0:
        return 0
    corr = (x * y).sum() / denom
    return corr.item()

def find_tensors(obj, path="root"):
    if isinstance(obj, torch.Tensor):
        print(f"TENSOR at {path}: shape={tuple(obj.shape)}, dtype={obj.dtype}")
