...
class GSM8KLoader(DataLoader):
    ...
    def __init__(self, questions: list[str], answers: list[str], random: bool = False) -> None:
        ...
        self.pre_prompt = """You must reply in exactly the following XML format and nothing else:
        ...
        - Do not write anything outside the <reasoning>...</reasoning> and <answer>...</answer> tags.
        - The <answer> section must contain a single integer (optionally with a leading minus sign) and no other characters.
        - Do not add extra tags, headings, or commentary."""
...
def build_gsm8k_dataloaders() -> Tuple[GSM8KLoader, GSM8KLoader]: 
    ...
    # Setup data loaders 
    trainloader = GSM8KLoader(train_questions.tolist(), train_answers.tolist(), random=True)
